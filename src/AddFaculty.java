...
import java.sql.*;
public class AddFaculty extends Frame implements ActionListener {
	Label l1,l2;
	TextField tf1,tf2;Button b1;
	AddFaculty(){
		setLayout(null);
		l1=new Label("Faculty ID(Should be rollno(First Year)):");
		...
		if(b1==e.getSource()){
			try{
			 ...
			 Connection myConn = DriverManager.getConnection("jdbc:mysql://localhost:3306/test","root","");
			   ...
			   myStmt.executeUpdate("insert into faculty values("+Integer.parseInt(tf1.getText())+",'"+tf2.getText()+"', 0 );");
